...
bool Game::replicateChoice(int choice)
{
	...
	case 3:
		if (isSplit())
		{
			...
			Card tmp(Suit::None, NameCard::None);
			...
			for (int cHand = 0; cHand < gambler->getcounthands(); cHand++)
			{
				...
				if (gambler->getHand()[cHand]->getCards()[1]->getName() == tmp.getName() && gambler->getHand()[cHand]->getCards()[1]->getSuit() == tmp.getSuit())
				{
					gambler->getHand()[cHand]->getCards().pop_back();
					gambler->getHand()[cHand]->takeCard(decks);
				}
			}
			return true;
